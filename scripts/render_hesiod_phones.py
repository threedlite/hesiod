#!/usr/bin/env python3
"""Phase 6: Hesiod text -> phones with the same columns as data/iliad/phones.csv, plus inventory and coverage checks."""
import csv, json, sys
...
def main():
    ...
    rows = list(csv.DictReader((ROOT / "data/hesiod/lines.csv").open()))
    ...
    for r in rows:
        ...
        out.append(dict(work=r["work"], n=r["n"], id=id_, n_syll=len(sy), phones=serialize(words),
                        ...
                        text=r["text_clean"]))
    with (ROOT / "data/hesiod/phones.csv").open("w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(out[0].keys())); w.writeheader(); w.writerows(out)
    nsyl = sum(int(o["n_syll"]) for o in out if o["n_syll"])
    print(f"{len(out)} lines, {sum(1 for o in out if o['phones'])} rendered, {nsyl} syllables -> data/hesiod/phones.csv")
    ...
    print("training phones unused by Hesiod:", sorted(train_inv - set(inv)) or "none")
