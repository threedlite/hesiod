#!/usr/bin/env python3
"""Phase 7: synthesize every Hesiod line, QC it automatically, and package for Classics Viewer.

  1. synth all lines of data/hesiod/phones.csv with the checkpoint (prediction mode) -> data/synth/hesiod_<name>/wav/<id>.wav
  ...
  3. reports/hesiod_qc.md; notes/human_qa_queue.md gets the failures
  4. --package: AAC-LC mono 44.1 kHz 96 kb/s MP4 in Hesiod/<Work>/book_1/line_<n>.mp4 and a zip
Usage: python scripts/synth_hesiod.py --ckpt train/runs/<run>/best.pt [--package] [--limit N]
"""
...
TITLES = {"Theogony": "Theogony", "Works and Days": "Works and Days", "Shield": "Shield of Heracles"}   # app work titles (confirm)
...
def synth(ckpt, ids, out, dur_scale=1.0):
    ...
    return run([PY, "train/fs2.py", "synth", "--ckpt", ckpt, "--list", str(lst), "--phones", "data/hesiod/phones.csv", "--out", str(out), "--dur_scale", str(dur_scale)])
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--ckpt", required=True); ap.add_argument("--limit", type=int, default=0); ap.add_argument("--package", action="store_true"); ap.add_argument("--package-only", action="store_true"); ap.add_argument("--name")
    ap.add_argument("--wav-dir", help="package these WAVs instead of <out>/wav (e.g. a converted voice)"); ap.add_argument("--package-name", help="zip/package label (default: hesiod_chamberlain_tts_<name>)")
    ...
    name = args.name or Path(args.ckpt).parent.name; out = ROOT / "data/synth" / f"hesiod_{name}"; out.mkdir(parents=True, exist_ok=True)
    phones = {r["id"]: r for r in csv.DictReader((ROOT / "data/hesiod/phones.csv").open()) if r["phones"]}
    ...
    L = ["# Hesiod synthesis QC: " + name, "", f"Lines synthesized: {len(ids)}. Corpus PER {100*vals.mean():.2f} % (median line {100*np.median(vals):.2f} %, p95 {100*np.percentile(vals,95):.2f} %).",
    ...
    for wk in ("Theogony", "Works and Days", "Shield"):
    ...
    (ROOT / f"reports/hesiod_qc_{name}.md").write_text("\n".join(L) + "\n"); print("\n".join(L[:8]))
    ...
    if bad:
        ...
        t += f"\n## Hesiod synthesized lines above PER {100*PER_MAX:.0f} % ({name})\n" + "".join(f"- {i} ({100*per[i]:.0f} %): {phones[i]['text']}\n" for i in sorted(bad, key=lambda i: -per[i]))
        qa.write_text(t)
    if args.package: package(out, ids, phones, name)

def package(out, ids, phones, name, wav_dir=None, label=None, lettered=False):
    """AAC MP4s in Hesiod/<Work>/book_1/line_<n>.mp4. Lettered line numbers (Theogony 929a-t) are
    skipped: the app stores them all as line 929, so only the unlettered line is addressable."""
    wav_dir = wav_dir or (out / "wav"); label = label or f"hesiod_chamberlain_tts_{name}"
    ...
    for i in ids:
        ...
        d = pkg / "Hesiod" / TITLES[r["work"]] / "book_1"; d.mkdir(parents=True, exist_ok=True)
