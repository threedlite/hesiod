...
A synthesized reading of Hesiod (Theogony, Works and Days, Shield of Heracles) in
reconstructed ancient Greek pronunciation, made by training a text-to-speech model
on David Chamberlain's line-by-line recording of the Iliad.
...
- Perseus Digital Library, canonical-greekLit, CC BY-SA 4.0: the Iliad and
  Hesiod texts.
...
| `reports/` | one report per phase: corpus reconciliation, phones, alignment and prosody baseline, scanner, Hesiod phones, TTS, Hesiod QC |
| `notes/decisions.md`, `notes/human_qa_queue.md` | decisions with their evidence; what only a listener can settle |
| `scripts/` | corpus QC, hypotactic parsing, transcript cleanup, split, MFA corpus, alignment QC, prosody baseline, Hesiod parse/scan/render/synthesis |
...
| `data/hesiod/`, `data/iliad/` | line tables, phone tables, metadata (audio and features are not in git) |
| `data/synth/hesiod_fs2_full/` | the Hesiod audio and the Classics Viewer package |
...
audio scores 2.4 % phone error rate on unseen Iliad lines and 3.1 % on Hesiod,
against 3.2 % for the real recordings.
...
`scripts/synth_hesiod.py --package`.
